...
public record SchemaImportResultResponse(
        int schemasImported,
        int tablesImported,
        int columnsImported,
        List<String> removedTableRefs,
        List<String> removedColumnRefs,
        List<SchemaImportResult.SchemaImportFailure> failures) {

    public static SchemaImportResultResponse from(SchemaImportResult result) {
        return new SchemaImportResultResponse(
                result.schemasImported(),
                result.tablesImported(),
                result.columnsImported(),
                result.removedTableRefs(),
                result.removedColumnRefs(),
                result.failures());
    }
}
