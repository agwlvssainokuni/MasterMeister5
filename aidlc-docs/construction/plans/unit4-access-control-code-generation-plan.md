...
### Step 1: 依存関係・設定の追加
- [ ] 1.1 `backend/build.gradle.kts`にCaffeine（`com.github.ben-manes.caffeine:caffeine`）、
      Jackson YAMLモジュール（`com.fasterxml.jackson.dataformat:jackson-dataformat-yaml`）を
      追加する

### Step 2: Business Logic Generation
- [ ] 2.1 `UserGroup`エンティティ（JPA）
- [ ] 2.2 `GroupMembership`エンティティ（JPA、`(groupId, userId)`一意制約）
- [ ] 2.3 `PermissionEntry`エンティティ（JPA、`SubjectType`enum（USER/GROUP）、
      ...
      columnName)`一意制約）
- [ ] 2.4 `UserGroupJpaRepository`/`GroupMembershipJpaRepository`/
      `PermissionEntryJpaRepository`（Subject条件のIN句検索メソッドを含む）
- [ ] 2.5 `AccessControlException`（業務例外、`ConnectionException`と同型パターン。
      グループ名重複・グループ未検出・Subject未解決・識別子不正・重複エントリ等）
- [ ] 2.6 `CacheKey`（値オブジェクト）、`PermissionCacheService`/
...
- [ ] 2.7 `EffectivePermission`（値オブジェクト、primaryLevel/canCreate/canDelete）
- [ ] 2.8 `PermissionYamlMapper`（`PermissionEntry`集合⇔YAML DTO変換、Jackson YAML
      モジュール使用）
- [ ] 2.9 `AccessControlService`/`AccessControlServiceImpl`（`setPrimaryPermission`/
...
- [ ] 2.10 Unit 3の`ConnectionSchemaServiceImpl#importSchema`を修正し、
...
- [ ] 3.1 `AccessControlServiceImplTest`（グループCRUD・カスケード削除、権限設定upsert、
      実効権限算出[ユーザ優先・グループ合成・階層フォールバック]、YAML全置換インポート・
      重複拒否・Subject未解決拒否）
- [ ] 3.2 `PermissionCacheServiceImplTest`（getCached/put、invalidateByUser/Group/
      Connectionの無効化範囲）
- [ ] 3.3 `PermissionYamlMapperTest`（往復変換の正当性）
- [ ] 3.4 property-based-testing拡張（jqwik）: functional-design/business-logic-model.mdの
      「テスト対象プロパティ」6件を実装する
- [ ] 3.5 Unit 3の`ConnectionSchemaServiceImplTest`に、`importSchema`成功時
      `invalidateByConnection`が呼び出されることを検証するテストケースを追加する

### Step 4: Business Logic Summary
- [ ] 4.1 `aidlc-docs/construction/unit4-access-control/code/business-logic-summary.md`
      を生成する

### Step 5: API Layer Generation
- [ ] 5.1 `GroupController`（一覧・作成・改名・削除、メンバー追加/削除）
- [ ] 5.2 `PermissionController`（既存権限参照、主権限/補助権限設定、YAMLエクスポート/
      インポート）
- [ ] 5.3 リクエスト/レスポンスDTO（record、SECURITY-05の入力検証アノテーション付与）
- [ ] 5.4 `GlobalExceptionHandler`更新: `AccessControlException`用のハンドラを追加
- [ ] 5.5 `SecurityConfig`確認: 既存の`/api/admin/**` → `hasRole("ADMIN")`ルールが
      新規エンドポイントをそのままカバーしており、追加変更は不要と確認する

### Step 6: API Layer Unit Testing
- [ ] 6.1 `GroupControllerTest`（作成/改名/削除、メンバー追加/削除、actorUserId伝播）
- [ ] 6.2 `PermissionControllerTest`（権限設定、YAMLエクスポート、YAMLインポート成功/
      検証エラー）

### Step 7: API Layer Summary
- [ ] 7.1 `aidlc-docs/construction/unit4-access-control/code/api-layer-summary.md`を
      生成する

### Step 8: Repository Layer Generation
- [ ] 8.1 `UserGroupJpaRepository`/`GroupMembershipJpaRepository`/
      `PermissionEntryJpaRepository`（Step 2で先行生成済み）

### Step 9: Repository Layer Unit Testing
- [ ] 9.1 各リポジトリの`@DataJpaTest`（一意制約、Subject条件のIN句検索、カスケード削除
      前提のクエリ）
...
- [ ] 11.1 APIクライアント関数（`api/groups.ts`、`api/permissions.ts`）
- [ ] 11.2 `GroupManagementScreen`（一覧、作成/改名モーダル、削除確認、メンバー管理）
- [ ] 11.3 `PermissionScreen`（接続選択、Subject選択、スキーマツリー表示、主権限/補助権限
       設定、YAMLエクスポート/インポート）
- [ ] 11.4 `App.tsx`ルーティング更新（`/groups`、`/permissions`、
       `RequireAuth role="ADMIN"`配下）、`AppLayout`更新（ADMIN限定navItem「グループ管理」
       「アクセス権限」を追加）
- [ ] 11.5 i18nメッセージ追加（ja/en、グループ管理・権限設定画面の全文言・エラーメッセージ）

### Step 12: Frontend Components Unit Testing
- [ ] 12.1 `GroupManagementScreen.test.tsx`（一覧表示、作成、メンバー追加/削除）
- [ ] 12.2 `PermissionScreen.test.tsx`（接続/Subject選択、ツリー表示、権限設定送信、
       YAMLインポートエラー表示）

### Step 13: Frontend Components Summary
- [ ] 13.1 `aidlc-docs/construction/unit4-access-control/code/frontend-summary.md`を
       生成する

### Step 14: Database Migration Scripts
- [ ] 14.1 `V11__create_user_group.sql`（Step 3のテスト前提として先行生成済み）
- [ ] 14.2 `V12__create_group_membership.sql`（同上）
- [ ] 14.3 `V13__create_permission_entry.sql`（同上）

### Step 15: Documentation Generation
- [ ] 15.1 `README.md`更新（グループ管理・アクセス権限設定画面の概要、必要であれば操作手順）

### Step 16: Deployment Artifacts Generation
- [ ] 16.1 デプロイ関連の環境変数追加は不要（Caffeineはプロセス内キャッシュのため設定項目
      なし）。念のため確認のみ行う
