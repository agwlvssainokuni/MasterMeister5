...
各対象RDBMSコンテナは初回起動時（データディレクトリが空の状態）に
`devenv/initdb/{mysql-mariadb,postgres}/01-schema-and-data.sql`を自動実行し、
動作確認用のサンプルスキーマ（顧客・商品・注文・注文明細、外部キー付き）と
サンプルデータ（顧客5,000件、商品200件、注文20,000件、注文明細約40,000件）を
投入する。ページング（デフォルト50件）・大量データ取得監査（デフォルト100件、
`MM5_BULK_ACCESS_THRESHOLD`）・クエリ実行結果の上限（1,000件、Unit 6）の動作を
実際のデータ量で確認できる。既にコンテナを起動済みで再投入したい場合は
`docker compose down`（該当サービスのコンテナ削除）してから起動し直すこと。
...
一般ユーザ・管理者を問わず「クエリ」画面でクエリビルダー（タブ形式のSELECT/FROM/JOIN/
WHERE/GROUP BY/HAVING/ORDER BY/LIMIT OFFSET入力、またはSQL直接入力）を使って任意の
読み取り専用SQLを作成・保存（公開範囲：全員/自分のみ）・パラメータ付きで実行でき、
「クエリ履歴」画面で過去の実行履歴を閲覧できる。管理者ダッシュボードの「監査ログ」
画面では、ユーザ管理・接続管理・アクセス権限・データ表示・クエリ実行など全Unitの
操作履歴をイベント種別・実行者・日時で絞り込んで閲覧できる（Unit 6）。クエリ実行結果は
最大1,000件・タイムアウト30秒で打ち切られ、閾値（デフォルト100件、
`MM5_BULK_ACCESS_THRESHOLD`環境変数で変更可能）以上のデータ取得は「データ表示」
画面の一覧取得と合わせて監査ログに記録される。追加の環境変数設定は不要
（`MM5_BULK_ACCESS_THRESHOLD`はデフォルト値のままで動作する）。
